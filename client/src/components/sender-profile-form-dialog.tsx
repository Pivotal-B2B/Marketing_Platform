...
export function SenderProfileFormDialog({ open, onOpenChange, profile }: SenderProfileFormDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: SenderProfileFormData) => {
      return await apiRequest("/api/sender-profiles", "POST", data);
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: SenderProfileFormData) => {
      return await apiRequest(`/api/sender-profiles/${profile?.id}`, "PUT", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/sender-profiles"] });
      toast({
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Pivotal CRM" {...field} data-testid="input-from-name" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
