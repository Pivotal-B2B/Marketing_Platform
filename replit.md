...
- **Verification System (Campaign-Based):** Configurable B2B CRM data verification and enrichment system with campaign-level eligibility rules, manual EmailListVerify validation, and strict quality controls. Features include:
    ...
    - **Smart Company Association:** 3-tier intelligent company matching during CSV upload: (1) Explicit domain field (highest priority), (2) Smart company name normalization (handles suffixes, abbreviations), (3) Email domain extraction (fallback). Ensures contacts are correctly associated with companies using normalized matching (e.g., "vietnamairlines.com" matches "Vietnam Airlines Ltd").
    ...
    - **AI-Powered LOCAL Office Enrichment:** Intelligent company data enrichment using Replit AI (GPT-4o) that searches for regional/local office information based on contact's location. Enrichment strategy: Searches for "Company Name" + "Contact Country" + "office" + "address" + "phone" to find local branches/subsidiaries, not global HQ. Features separate confidence scoring for address (≥0.7) and phone (≥0.7) with independent gating, complete address detection (requires all core fields), partial success preservation during errors, and country-specific formatting for 30+ countries including Singapore, Vietnam, UK, USA, etc.
