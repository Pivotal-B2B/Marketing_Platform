...
- **Compliance & Suppression:** Multi-tier suppression system (campaign-level and global) for emails (unsubscribe) and phones (Do Not Call) with RESTful API support. **Campaign-Level Suppression with Dynamic Account Cap Enforcement**: Comprehensive suppression checking with contact ID, account ID, email, domain (from email), and company name matching. Smart normalization for domains (removes www, TLD parsing) and company names (removes Inc, Ltd, LLC). Special cap=1 rule: when account cap is 1, ANY suppressed contact blocks entire account. Dynamic cap enforcement supports queue_size, connected_calls, and positive_disp modes. **Auto-Suppression on Qualified/Lead Disposition**: When a call is marked as "Qualified" or "Lead" (or any disposition with systemAction='converted_qualified'), the contact is automatically added to campaign suppression and account cap is enforced. If cap is reached, entire account is suppressed and removed from all queues. Campaign-level phone suppression supports CSV upload and manual entry for accounts, contacts, and domains. Informational wizard step guides users to configure suppressions post-creation via the Suppressions tab in campaign edit page.
...
- **Best Phone & Best Address Selection with Strict Country Matching:** Intelligent data selection system that prevents cross-country data assignments. **Best Phone Priority:** CAV Tel > Mobile > Contact Phone > AI Enriched Phone > Company HQ Phone. **Best Address Priority:** CAV Custom Fields > Contact Address > AI Enriched Address > Company HQ Address. **Critical Country Matching Rules:** (1) CAV/Mobile/Contact phones assumed to be in contact's country (lenient), (2) AI enriched phone compared against `aiEnrichedCountry` with lenient fallback if missing, (3) HQ phone STRICTLY rejected if `hqCountry` mismatches contact country (prevents UK contact getting USA HQ phone), (4) Address completeness requires line1 + city + (state OR postal), (5) HQ address STRICTLY rejected if country mismatch (prevents UK contact getting USA HQ address), (6) CAV custom fields allowed to have missing country (verified data, inferred from contact). Implementation in `server/lib/verification-best-data.ts` reuses `doesCountryMatch()` helper for consistent country validation across phone and address selection.
