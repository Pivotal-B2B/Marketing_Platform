...
- **Campaign Management:**
    ...
    - **Telemarketing - Dual Dialer Strategy (PRODUCTION READY):**
        ...
        - **Power Dial Mode:** Automated dialing with AMD decision pipeline (confidence threshold 0.70), progressive/predictive/preview modes, business hours enforcement, PID-like pacing engine with abandon-rate feedback (3% target), human-only agent routing. Powered by PowerDialerEngine with dynamic dial ratio adjustment.
        ...
        - **API Endpoints:** POST /campaigns/:id/dial-mode, POST /campaigns/:id/manual/queue/add (filters or IDs), POST /campaigns/:id/manual/queue/pull, POST /campaigns/:id/power/settings (AMD config), POST /telephony/events/amd (webhook), GET /campaigns/:id/pacing-metrics.
