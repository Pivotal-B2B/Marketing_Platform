...
- **Contact Deduplication/Upsert Logic**: Implemented intelligent duplicate prevention in CSV import system. When importing contacts, the system now checks if a contact with the same email already exists and updates it instead of creating a duplicate. Import results now show separate counts for created vs updated contacts. This prevents data duplication and maintains data integrity while allowing users to refresh contact information via CSV imports.
- **Admin-Only Contacts Export**: Restricted contacts export functionality to admin users only for enhanced data security and compliance. Main export button and bulk export button now check for admin role in both legacy `role` field and new `roles` array. Removed duplicate non-functional export button. Non-admin users (agents, campaign managers, quality analysts) no longer see export options on contacts page.
- **WebRTC Connection Fix**: Resolved critical WebRTC connectivity issues preventing Agent Console from establishing Telnyx voice connections. Fixed Content Security Policy (CSP) to allow connections to Telnyx servers (wss://sip.telnyx.com, https://*.telnyx.com), configured SDK to use TCP/TLS transport instead of UDP (better for cloud environments via iceTransportPolicy: 'relay'), and updated SIP credentials in database to resolve "Login Incorrect" authentication errors. Agent Console now successfully connects to Telnyx WebRTC service.
- **Location/Address Field Visibility Enhancement**: Enhanced contact and account detail pages to always display location/address fields in main grid, eliminating conditional rendering that hid these fields when empty. Contact detail page now shows City, State, County, Postal Code, Country fields in main grid with "-" fallbacks; Street Address and Full Location String remain in conditional section. Account detail page now displays HQ City, HQ State, HQ Postal Code, HQ Country as individual always-visible fields in main grid (previously combined into single "Headquarters" field); Year Founded also made always-visible. Street address lines remain conditionally rendered to avoid clutter when empty.
- **Critical Role Name Fix**: Fixed critical bug where code was checking for 'qa_analyst' role but database defined 'quality_analyst' role, preventing QA analysts from accessing their features. Updated all role checks across frontend (dashboard.tsx, app-sidebar.tsx, lead-detail.tsx) and backend (routes.ts, validation schemas) to use correct 'quality_analyst' role name matching database enum. QA analysts can now access QA & Leads section in sidebar and see full dashboard instead of agent-limited view.
- **Multi-Role RBAC Fix**: Fixed critical bug where users with multiple roles (e.g., Agent + QA Analyst) only saw agent-limited features instead of full dashboard access. Dashboard now correctly shows agent view ONLY for users with agent role and no elevated roles; multi-role users (agent + quality_analyst/admin/campaign_manager) now see full dashboard with all features.
