...
- **Data Management:** Unified CSV Import/Export System with smart field mapping and optimized bulk database operations (inserts, updates, suppressions). **Platform-Wide Dynamic Filters (PRODUCTION READY):** Config-driven filter system with collapsible category accordions, multi-select dropdowns, async type-ahead search, and date range pickers. Filter options pull from actual database values (accounts.industry_standardized, accounts.employees_size_range, accounts.annual_revenue) for dynamic, real-world data instead of static reference tables. RBAC enforcement limits filter visibility by role. Supports saved segments, selection caps, and real-time filter chip display with removal. Components: FilterShell (Sheet UI with accordion categories showing active filter counts), MultiSelectFilter (Popover with checkboxes and chips), AsyncTypeaheadFilter (scoped Country→State→City filtering), DateRangeFilter (presets + custom ranges), ChipsBar (active filter pills). API routes: GET /api/filters/options/{source} endpoints with pagination, search, caching (industries, company-sizes, company-revenue, seniority-levels, job-functions, departments, technologies, countries, states, cities, users).
