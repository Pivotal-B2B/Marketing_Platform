...
- **Data Management:** Unified CSV Import/Export System with intelligent deduplication/upsert logic, platform-wide dynamic filters, and RBAC-enforced filter visibility. Domain Sets support enhanced CSV format with company names (domain,account_name,notes) enabling hybrid matching by both domain and company name for improved account identification. **Smart Company Name Normalization:** Handles company name variations, legal suffixes (Ltd, Inc, LLC, Corp, plc, etc.), punctuation, and common abbreviations (Airways→Air, International→Intl) for accurate account matching and deduplication across all data imports. **Dynamic Custom Fields System:** Full-featured custom field infrastructure allowing users to create and manage custom fields for both contacts and accounts during CSV import mapping or via Settings UI. Features include: in-mapping field creation (create new custom fields directly from CSV field mapper with auto-suggested field keys and labels), auto-registration (unmapped CSV columns automatically registered as custom fields after import), management UI (rename, configure types, activate/deactivate fields in Settings), and smart export (CSV exports use custom field display labels instead of raw keys with proper escaping).
...
- **Verification System (Campaign-Based):** Configurable B2B CRM data verification and enrichment system with campaign-level eligibility rules, manual EmailListVerify validation, and strict quality controls. Features include:
    ...
    - **Strict 4-Rule Suppression System:** Advanced suppression matching with strict rules to prevent over-suppression. Contacts are suppressed ONLY when ONE of these conditions matches: (1) Email exact match (case-insensitive), (2) CAV ID exact match, (3) CAV User ID exact match, or (4) BOTH Full Name AND Company match together (requires all fields non-empty). Partial matches (first name only, last name only, company only) are explicitly forbidden. Implementation uses SHA256 hashing for name+company combinations with normalized columns (email_norm, full_name_norm, company_norm, name_company_hash) and database indexes for efficient bulk checking. RESTful APIs support list, bulk check, add, and remove operations with Zod validation.
    ...
    - **Bulk Field Update:** Enterprise-grade bulk update feature with strict security controls allowing mass updates to 21 contact fields (addresses, job info, contact details). Features: strict field allowlist mapping frontend names to DB columns, 500-character value limits, input sanitization, comprehensive audit logging of all bulk operations, RBAC enforcement, and automatic updatedAt tracking.
