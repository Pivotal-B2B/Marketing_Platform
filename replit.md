...
- **Campaign Management:**
    ...
    - **Telemarketing - Dual Dialer Strategy:**
        - **Manual Dial Mode:** Agent-driven queue with pull/lock workflow, campaign-level collision prevention (partial unique index), dual suppression checking (email & phone), filter-based audience selection, and retry rules. Powered by ManualQueueService with atomic queue management.
        - **Power Dial Mode:** Automated dialing with AMD/voicemail detection, progressive/predictive/preview modes, business hours enforcement, pacing engine with abandon-rate feedback, human-only agent routing. Powered by PowerDialerEngine (in development).
        - **AMD & Voicemail:** Advanced Machine Detection with confidence scoring (0.00-1.00), VoicemailPolicyExecutor for TTS/audio message delivery, campaign/contact-level VM caps, cooldown enforcement, callback scheduling, and restricted region blocking.
